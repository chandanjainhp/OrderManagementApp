import { PASSWORD_RESET_REQUEST_TEMPLATE, PASSWORD_RESET_SUCCESS_TEMPLATE, VERIFICATION_EMAIL_TEMPLATE, PASSWORD_RESET_OTP_TEMPLATE, ORDER_UPDATE_TEMPLATE } from "./emailTemplates.js";
...
// Export sendEmail for external use
export { sendEmail };
...
export const sendOrderUpdateEmail = async (email, orderDetails) => {
  console.log(`Sending order update email to ${email}...`);  // Debug log

  const { customerName, orderId, orderStatus, totalAmount } = orderDetails;

  const htmlContent = ORDER_UPDATE_TEMPLATE
  ...
  return sendEmail({
    to: email,
    subject: `Order Update #${orderId} - Order Management App`,
    html: htmlContent,
